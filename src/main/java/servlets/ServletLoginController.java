...
public class ServletLoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( user != null && !user.isEmpty() && password != null && !password.isEmpty()) {
			...
			if (auth.login()) {
				if(request.getSession().getAttribute("logged") == null || (request.getSession().getAttribute("logged") != null && request.getSession().getAttribute("logged").toString().isEmpty())) {
					request.getSession().setAttribute("logged", auth.getUser());
				}
				
				if(url == null || url.equals("null")) url = "admin/dashboard.jsp";
